...
import { useFetchGestionRoles, GestionRolesItem } from './fetchers/useFetchGestionRoles';
import { useCreateGestionRoles, useUpdateGestionRoles, useDeleteGestionRoles } from './fetchers/useCreateGestionRoles';
import { validateGestionRoles, ValidationError } from './checkers/validators';
import { useAlertStore } from '@/stores/alertStore';

export default function GestionRolesPage() {
  const { data: items, loading, fetch } = useFetchGestionRoles();
  const { create, loading: creating } = useCreateGestionRoles();
  const { update, loading: updating } = useUpdateGestionRoles();
  const { deleteItem, loading: deleting } = useDeleteGestionRoles();
  const { showConfirm } = useAlertStore();

  const [open, setOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<GestionRolesItem | null>(null);
  ...
  const handleOpen = (item?: GestionRolesItem) => {
    if (item) {
      setEditingItem(item);
  ...
  const handleSubmit = async () => {
    const validationErrors = validateGestionRoles(formData);
  ...
  const handleDelete = (item: GestionRolesItem) => {
    showConfirm(
      'Supprimer le rôle',
    ...
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1400, mx: 'auto' }}>
      ...
      <Box sx={{
        ...
        <Box>
          ...
          <Typography variant="body1" color="text.secondary" sx={{ opacity: 0.8 }}>
            Définissez et gérez les niveaux d'accès de votre plateforme
          </Typography>
        </Box>
        ...
        <Table sx={{ minWidth: 800 }}>
          <TableHead>
            <TableRow sx={{ bgcolor: 'grey.50' }}>
              <TableCell sx={{ fontWeight: 700, color: 'text.secondary', py: 2.5 }}>Rôle</TableCell>
              ...
              <TableRow>
                <TableCell colSpan={3} sx={{ py: 10, textAlign: 'center' }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Commencez par créer un nouveau rôle système.
                  </Typography>
                </TableCell>
              </TableRow>
            )}
      ...
      >
        <DialogTitle sx={{ fontWeight: 800, fontSize: '1.5rem', pb: 1 }}>
          {editingItem ? 'Modifier le rôle' : 'Nouveau rôle'}
        </DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            {editingItem
              ? "Modifiez les informations du rôle ci-dessous."
              : "Remplissez les informations pour créer un nouveau rôle système."}
          </Typography>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <TextField
              ...
              disabled={creating || updating}
              placeholder="Décrivez les responsabilités et les accès de ce rôle..."
              InputProps={{
                sx: { borderRadius: 2.5, bgcolor: 'grey.50' }
              }}
            />
